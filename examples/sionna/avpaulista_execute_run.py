...
for current_step in range(number_of_steps):
    ...
    # Create transmitter
    tx = Transmitter(
        name="tx", position=[rx_starting_x + 10, rx_starting_y, rx_starting_z]
    )
    ...
    my_cam = Camera(
        "my_cam",
        position=[rx_starting_x, rx_starting_y, rx_starting_z + 150],
        look_at=[0, 0, 90],
    )
    scene.add(my_cam)
