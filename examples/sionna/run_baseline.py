def getTopK(topK, dataset):
    ...
    for scene in range(number_of_scenes):  # iterates over all scenes
        ...
        output.append(
            [
                str(np.argwhere(k == channel_magnitudes_in_scene)[0])
...
# output_filename = os.path.join(current_dir, "allruns03.npz")
output_filename = os.path.join(current_dir, "nlos.npz")
# output_filename = os.path.join(current_dir, "nlos.npz")
...
best_ray_true = [str(ray[0]) for ray in best_ray]
...
# for rx in range(4):
#     for tx in range(64):
#         pairs.append(str([rx, tx]))

enc = LabelEncoder()
enc.fit(topk_pairs_per_scene.flatten())
...
plt.savefig("v2conf_mat_rnd-forest_200estim.png")

plt.figure()
plt.hist(y_test, bins=number_of_classes, label="y_test")
plt.hist(y_pred, bins=number_of_classes, label="y_pred", alpha=0.3, edgecolor="white")
plt.xticks(range(number_of_classes), rotation=90)
plt.legend()
# plt.grid()
plt.savefig("v2label_hist_rnd-forest_200estim.png", dpi=200)
